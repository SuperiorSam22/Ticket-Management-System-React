...
router.post(
  ...
  async (req, res) => {
    try {
      const { subject, sender, message, severity } = req.body;
...
router.post("/", async (req, res) => {
  const { subject, message, issueDate, severity } = req.body;
  ...
  try {
      const newTicket = new Ticket({
          ...
          message,
          issueDate,
          severity,// New field for assigned user
      });

      await newTicket.save();
      res.json({ status: "success", message: "Ticket created successfully", result: newTicket });
  } catch (error) {
